...
| 2026-09-15 | Sửa bước 4 `DEPLOY.md` cho PowerShell: `node scripts/gen-vapid.cjs` tự ghi `.vapid.env` UTF-8 (tránh `>` → UTF-16 khiến `secrets set` báo No arguments). Đã set secrets thành công (`count:3`) | Còn dán `VITE_VAPID_PUBLIC_KEY` vào `.env` / Vercel | Bước 8 `.env` → bước 10 Vercel nếu chưa |
| 2026-09-15 | Thêm template email OTP theo theme app: `docs/design/frontend/ui/otp-email.html` (màu hồng đất + Be Vietnam Pro, `{{ .Token }}`) | Cần SMTP xong rồi dán Subject/Body vào Supabase Magic Link | Bật SMTP → dán template → thử gửi OTP |
| 2026-09-15 | Sửa Home "Mời lại": lấy/sinh lại mã mời nếu thiếu, `shareInvite` fallback copy + toast phản hồi (trước đó bấm im lặng / nút xám khi chưa có code) | Chờ thử trên máy | Thử Mời lại trên Home khi space 1 người |
| 2026-09-15 | Fix `peek_invite`: bỏ `STABLE` (hàm INSERT `invite_attempts` → lỗi *INSERT is not allowed in a non-volatile function* trên Join). Migration `20260929090000` | Đã push migration | Thử lại link/mã mời Join |
